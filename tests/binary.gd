...
func _init() -> void:
	...
	data.dictionary_variant = { "int": 1, "float": 1.0, "vector": Vector2(1, 2) }
	...
	var deserialized: Data = ObjectSerializer.binary.deserialize(bytes_to_var(var_to_bytes(serialized)))
